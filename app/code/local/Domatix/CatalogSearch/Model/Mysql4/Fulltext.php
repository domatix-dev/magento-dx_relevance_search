<?php
class Domatix_CatalogSearch_Model_Mysql4_Fulltext extends Mage_CatalogSearch_Model_Mysql4_Fulltext {
	protected function _construct() {
		$this->_init('domatixcatalogsearch/fulltext', 'product_id');
	}
	...
	public function prepareResult($object, $queryText, $query) {
		if (!$query->getIsProcessed()) {
			...
			// If stock_relevance parameter is set to true items out of stock will have 0 relevance
			if ($this->_getStockRelevance()) {
				$relevanceExpression = 'IF (`d`.`stock_status` , '.$relevanceExpression.' ,0)'; 
			} 
			...
			$sql = sprintf("INSERT INTO `{$this->getTable('catalogsearch/result')}` "
										 ...
										 . "INNER JOIN `{$this->getTable('cataloginventory/stock_status')}` AS `d` "
										 . "ON `s`.`product_id`=`d`.`product_id` "
										 . "WHERE (%s%s%s) AND `s`.`store_id`='%d')"
	...
	protected function _getPriorityAttributes($priorityIndex) {
		if (is_null($this->_priorityAttributes)) {
			...
			for ($i = 0; $i < self::NUMBER_OF_PRIORITY_SEARCH_ATTRIBUTES; $i++) {
				$attributeCodes = explode(',', Mage::getStoreConfig('catalog/domatixcatalogsearch/priority_attributes_'.($i+1)));
	...
	protected function _getPriorityWeight($index) {
		if (is_null($this->_priorityWeights)) {
			...
			for ($i = 0; $i < self::NUMBER_OF_PRIORITY_SEARCH_ATTRIBUTES; $i++) {
				$value = Mage::getStoreConfig('catalog/domatixcatalogsearch/priority_weight_'.($i+1));
	...
	protected function _getNegateRelevance() {
		$value = Mage::getStoreConfig('catalog/domatixcatalogsearch/negate_relevance');
		return (bool)$value;
	}
	...
	protected function _getStockRelevance() {
		$value = Mage::getStoreConfig('catalog/domatixcatalogsearch/stock_relevance');
		return (bool)$value;
	}
}
